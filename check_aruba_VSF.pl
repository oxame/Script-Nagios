...
my $OID_VSFTATEMENBER = 'HP-VSF-VC-MIB::hpicfVsfVCMemberState';
...
my $Command = "snmpwalk -v $ARGV[0] -c $ARGV[1] $ARGV[2] $OID_VSFTATEMENBER";
my $result = `$Command`;
#$result =~ m/IF-MIB::ifDescr\.(\d+)\s=\s.*$interface.*/;
my $Menber = -1;
my $MenberMissing = 0;
foreach(split("HP-VSF-VC-MIB::hpicfVsfVCMemberState.",$result))
{
$Menber++;
	if($_ =~ "missing")
	{
